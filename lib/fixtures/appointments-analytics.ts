...
export function fixture(params?: Record<string, string | number | undefined>): Envelope<unknown> {
  ...
  const data = {
    ...
    status_breakdown: [
      { status: 'Done', count: 2, gross_bhd: 35.9 },
      { status: 'Confirmed', count: 1, gross_bhd: 18.0 },
    ],
    type_distribution: [
      { type_raw: 'Novo Instant', type_normalized: 'novo instant', count: 1, track: 'standard' },
      { type_raw: 'Scheduled', type_normalized: 'scheduled', count: 1, track: 'standard' },
      { type_raw: 'Novo Scheduled', type_normalized: 'novo scheduled', count: 1, track: 'standard' },
    ],
    recent,
