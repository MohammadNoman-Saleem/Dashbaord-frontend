// GET /api/funnels/uiux - the UI/UX behaviour tab (dead clicks and frustration
// signals). Ported from the NestJS FunnelsController.uiux. Thin: resolve the
// viewer, resolve the throttled ?refresh=1 cache bypass, call the
...
// the backstop only. The FunnelUiuxData shape (lib/api/contract.ts) is
...
export const GET = handler(async (req, ctx) => {
  ...
  const bypass =
    refresh === '1' && getRefreshThrottle().allow(viewer.key, 'funnels.uiux');
  const { data, parts } = await getFunnelsService().uiux(bypass);
